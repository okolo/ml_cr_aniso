"""
Copied from PyAstronomy's angularDistance implementation
 see https://pyastronomy.readthedocs.io/en/latest/pyaslDoc/aslDoc/angularDistance.html

Note: PyAstronomy is incompatible with recent numpy releases,
thus the getAngDist function has been copied to the project dependencies.
"""
import numpy as np
...
def getAngDist(ra1, dec1, ra2, dec2):
    ...
    dist = 2.0 * np.arcsin(np.sqrt(np.sin(delt_lat / 2.0) ** 2 + \
                                   np.cos(dec1 * np.pi / 180.) * np.cos(dec2 * np.pi / 180.) * np.sin(
        delt_lon / 2.0) ** 2))
